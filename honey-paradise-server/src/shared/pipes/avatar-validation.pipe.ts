...
import {
  ALLOWED_AVATAR_FILE_FORMATS,
  ALLOWED_AVATAR_FILE_SIZE,
...
export class AvatarFileValidationPipe implements PipeTransform {
  ...
  public async transform(value: any, metadata: ArgumentMetadata) {
    ...
    if (!filename)
      throw new BadRequestException(this.i18n.t("d.errors.files.avatar.empty"));
    ...
    if (!isFileFormatValid)
      throw new BadRequestException(
        this.i18n.t("d.errors.files.avatar.unsupported_type"),
      );
    ...
    if (!isFileSizeValid)
      throw new BadRequestException(
        this.i18n.t("d.errors.files.avatar.too_big"),
      );
