...
export class AccountService {
	...
	async getTelegramInfo(userId: string): Promise<IGetTelegramInfoResponse> {
		...
		if (!user.telegramId)
			throw new BadRequestException(
				this.i18n.t("d.errors.account.telegram_not_connected")
			)

		const { username } = await this.telegramService.getTgUsername(
			+user.telegramId
		)
	...
	async disconnectTelegram(userId: string): Promise<boolean> {
		...
		await this.notificationsService.updateSettings(userId, {
		...
		await this.notificationsService.send(
			userId,
			"Telegram аккаунт был только что отвязан",
	...
	async create(
		...
		if (isEmailExist)
			throw new BadRequestException(this.i18n.t("d.errors.email.is_exist"))

		const user = await this.createNew(
			{ ...dto, avatarPath: DEFAULT_AVATAR_PATH },
			true
		)
	...
	async createNew(dto: Partial<User & Provider>, isRegister: boolean = false) {
		...
		const isUsernameExist = username
			? await this.profileService.getProfile(username, "username")
			: false
		if (isUsernameExist && isRegister)
			throw new BadRequestException(this.i18n.t("d.errors.username.is_exist"))

		const new_username =
			typeof username === "undefined"
				? await this.getUsernameFromEmail(email)
				: username
		...
		await this.prisma.provider.create({
			data: {
				...
				type: type
					? (type.toUpperCase() as EnumProviderTypes)
					: EnumProviderTypes.CREDENTIALS
			}
		})
	...
	async sendEmailVerificationCode(
		...
		const token = await this.verificationService.sendVerificationEmail(
			req,
			userAgent,
			user
		)
		...
		if (
			...
			await this.telegramService.sendEmailConfirmationCode(
	...
	async updatePassword(id: string, password: string, req: Request) {
		const { isTFAEnabled } = await this.profileService.updatePassword(
			id,
			password
		)
		...
		await this.notificationsService.send(
			id,
			"На вашем аккаунте был только что изменен пароль",
			EnumNotificationType.ACCOUNT_STATUS
		)
	...
	private async getUsernameFromEmail(email: string): Promise<string> {
		...
		const isUsernameExist = await this.profileService.getProfile(
			emailUsername,
			"username"
		)
