...
export class VerificationService {
	...
	async verifyEmail(
		...
		const existingTokens = await this.prisma.token.findMany({
			where: { type: EnumTokenTypes.EMAIL_VERIFY }
		})
	...
	async recoverPassword(dto: UpdatePasswordDto) {
		...
		const existingTokens = await this.prisma.token.findMany({
			where: { type: EnumTokenTypes.PASSWORD_RECOVERY }
		})
	...
	async verifyTFA(res: Response, dto: AuthTfaDto) {
		...
		const existingTokens = await this.prisma.token.findMany({
			where: { type: EnumTokenTypes.TFA_VERIFY }
		})
	...
	async verifyTelegramConnectToken(token: string, chatId: number) {
		const exitingTokens = await this.prisma.token.findMany({
			where: { type: EnumTokenTypes.TELEGRAM_CONNECT }
		})
	...
	async sendTelegramAuthToken(chatId: number) {
		...
		const token = await this.createToken(
			user.id,
			EnumTokenTypes.TELEGRAM_TFA_AUTH,
			ms("10min")
		)
	...
	async connectTelegram(userId: string): Promise<{ url: string }> {
		...
		const token = await this.createToken(
			user.id,
			EnumTokenTypes.TELEGRAM_CONNECT,
			ms("30m")
		)
	...
	async sendTFACode(
		...
		const token = await this.createToken(
			user.id,
			EnumTokenTypes.TFA_VERIFY,
			ms("30m")
		)

		const metadata = getSessionMetadata(req, userAgent)

		await this.mailService.sendTFAEmail(
			user.email,
			token,
			user.username,
			metadata
	...
	async sendRecoverPasswordEmail(
		...
		if (!user)
			throw new NotFoundException(
				this.i18n.t("d.errors.account.not_found_email")
			)

		const token = await this.createToken(
			user.id,
			EnumTokenTypes.PASSWORD_RECOVERY,
			ms("30m")
		)
	...
	private async validateToken(
		tokens: Token[],
		code: string,
		type: "default" | "password-recover" = "default"
	) {
		...
		if (!token) {
			if (type === "default") {
				throw new BadRequestException(
					this.i18n.t("d.errors.verification.invalid_code")
				)
			} else {
				throw new BadRequestException(
					this.i18n.t("d.errors.verification.password_recovery_token_missing")
				)
			}
		}
		...
		if (!isExpired)
			throw new BadRequestException(
				this.i18n.t("d.errors.verification.code_expired"),
				{ cause: EnumErrorCauses.EMAIL_TOKEN_EXPIRED }
			)
