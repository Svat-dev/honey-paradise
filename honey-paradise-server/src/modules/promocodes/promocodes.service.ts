...
export class PromoCodesService {
	...
	async createPromoCode(dto: CreatePromoCodeDto): Promise<boolean> {
		...
		if (token)
			throw new BadRequestException("Promo token with this code already exists") // TODO: translation
	...
	async usePromoCode(userId: string, dto: UsePromoCodeDto): Promise<boolean> {
		const { birthdate, _count, cart } = await this.prisma.user.findUnique({
			...
			select: {
				...
				cart: {
					select: { id: true, promoTokens: true, usedPromoTokens: true }
				}
			}
		})
		...
		if (
			token.status === EnumPromoTokensStatus.APPLIED ||
			cart.promoTokens.includes(token.id)
		)
			throw new ConflictException("Promo code is already applied") // TODO: translation

		if (
			token.status === EnumPromoTokensStatus.USED ||
			cart.usedPromoTokens.includes(token.type)
		)
			throw new ConflictException("Promo code is already used") // TODO: translation

		if (
			(token.expiresAt && token.expiresAt < new Date()) ||
			token.status === EnumPromoTokensStatus.EXPIRED
		)
			throw new ConflictException("Promo code is expired") // TODO: translation

		if (token.type === EnumPromoTokenTypes.BIRTHDAY) {
			if (!birthdate)
				throw new ConflictException("You didn't specify your birthdate") // TODO: translation
			...
			if (month !== today.getMonth() || day !== today.getDate())
				throw new ConflictException("You don't have birthday today!") // TODO: translation
		} else if (token.type === EnumPromoTokenTypes.FIRST_ORDER) {
			if (_count.orders > 0)
				throw new ConflictException("You have more than one order!") // TODO: translation
			...
			if (startDate > new Date() || endDate < new Date())
				throw new ConflictException("Promo code is not valid now") // TODO: translation
		...
		await this.prisma.cart.update({
			...
			data: {
				...
				usedPromoTokens:
					token.type === "PERSONAL" ? undefined : { push: token.type }
			}
		})
	...
	async countDiscounts(
		cartId: string
	): Promise<{ discount: number; freeDelivery: boolean }> {
		try {
			...
			const promoTokens = await this.prisma.promoToken.findMany({
				where: {
					id: { in: tokensIds },
					status: {
						notIn: [EnumPromoTokensStatus.EXPIRED, EnumPromoTokensStatus.USED]
					}
				},
