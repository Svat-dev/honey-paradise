...
export class NotificationsService {
	...
	async markAsRead(dto: NotificationsIdsDto): Promise<boolean> {
		...
		for (const { id, isRead } of notifications) {
			if (isRead && single) throw new ConflictException(this.i18n.t("d.errors.notifications.already_read"));
	...
	async markAsArchived(dto: NotificationsIdsDto): Promise<boolean> {
		...
		for (const { id, isRead, archivedAt } of notifications) {
			if (!isRead) {
				if (single) throw new ConflictException(this.i18n.t("d.errors.notifications.not_read_to_archive"));
				else continue;
			}

			if (archivedAt && single) throw new ConflictException(this.i18n.t("d.errors.notifications.already_archived"));
