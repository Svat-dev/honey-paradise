...
const DynamicSearchInput = dynamic(() =>
	import("./SearchInput").then(mod => mod.SearchInput)
)
...
const MiddlePart = () => {
	...
	return (
		<div className="relative flex w-full flex-row-reverse items-center">
			<Button
				variant="secondary"
				className="z-40 gap-2 !rounded-lg p-2"
			...
			<DynamicSearchInput
				...
				form={searchForm}
				setIsOpen={setIsOpen}
				setOverlay={setSearchOverlay}
			/>

			<AnimatePresence>
				{isOpen && searchOverlay && <PresearchDataBlock term={term} />}
			</AnimatePresence>
