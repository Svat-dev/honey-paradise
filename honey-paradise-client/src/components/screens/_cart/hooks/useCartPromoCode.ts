import { TPromoCodeFields, promoCodeSchema } from "@/shared/lib/schemas/promo-code.schema";
...
export const useCartPromoCode = () => {
	...
	const onSubmit = async (data: TPromoCodeFields) => {
		try {
			...
			if (errMsg) form.setError("promoCode", { message: errMsg, type: "validate" }, { shouldFocus: true });
		}
	};
