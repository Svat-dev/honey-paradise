...
export const useAvatarFrameDialog = (framePath: string | undefined | null) => {
	...
	const [url, setUrl] = useState<string | null>(
		typeof framePath === "undefined" ? null : framePath
	)

	const staticFrames: IFrameData[] = useMemo(
		() =>
			[...new Array(6).keys()].map((_, i) => ({
				label: t("labels.static", { i }),
				value: i
			})),
		[locale]
	)
	...
	useEffect(() => {
		...
		if (framePath) {
			...
			setFrame(
				framePath.includes("/animated/") ? Number(second) + 5 : Number(second)
			)
		}
	}, [framePath])
...
export const useAvatarFrameDialogPreview = () => {
	...
	const { updateAvatarFrameAsync, isAvatarFrameUpdating } =
		useUpdateAvatarFrameS()
