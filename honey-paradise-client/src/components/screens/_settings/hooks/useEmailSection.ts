...
export const useEmailSection = (email: string | undefined) => {
	...
	const [emailUnique, setEmailUnique] = useState<boolean | "loading" | null>(
		null
	)
	...
	const confirmEmail = () => {
		...
		push(
			`${EnumAppRoute.EMAIL_CONFIRMATION}&utm_source=${EnumAppRoute.ACCOUNT}`
		)
	}
	...
	useEffect(() => {
		...
		if (value === form.formState.defaultValues?.email || !checkEmail(value))
	...
	useDebounce(
		async () => {
			...
			const isDisabled =
				value.length === 0 ||
				!checkEmail(value, "unique-check") ||
				form.formState.defaultValues?.email === value
