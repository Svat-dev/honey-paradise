...
export const useNotificationsSettings = () => {
	...
	const tgt = useTranslations(
		"global.settings.content.profile.telegram-linking"
	)
	...
	const onSwitchChange = async (
		value: boolean,
		field: keyof UpdateNotificationsSettingsDto
	) => {
		try {
...
export const usePermissionSection = () => {
	...
	const onRequestPermission = () =>
		Notification.requestPermission()
			...
			.catch(() =>
				console.error("Error requesting permission to send notifications")
			)
