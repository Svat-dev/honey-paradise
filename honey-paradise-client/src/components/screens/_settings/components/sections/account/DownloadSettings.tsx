  ...
  return (
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      <DropdownMenuTrigger asChild>
        <Button variant="default" className="items-center gap-2 px-2 py-1.5" title={t("labels.saveFileBtn")} disabled={isLoading}>
      ...
      <DropdownMenuContent>
        {data.map(({ title, label, fn }) => (
          <DropdownMenuItem key={title} asChild>
            <Button variant="ghost" onClick={() => fn()} className="w-full px-2 py-2 gap-1 !justify-start hover:!bg-muted/20" title={label}>
