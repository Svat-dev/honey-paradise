...
const FrameDialogFrameItem: FC<IProps> = ({
	value,
	label,
	frame,
	setFrame
	...
	const handleMouseEnter = (e: MouseEvent<HTMLImageElement>) => {
		e.currentTarget.setAttribute(
			"src",
			getFramesPath(`${EnumStaticRoute.ANIMATED_FRAMES}/${value - 5}.png`)
		)
	}
	...
	const handleMouseLeave = (e: MouseEvent<HTMLImageElement>) => {
		e.currentTarget.setAttribute(
			"src",
			getFramesPath(`${EnumStaticRoute.FRAMES}/${value}.webp`)
		)
	}
	...
	return (
		<button
			className={cn(
				"aspect-square size-24 rounded-lg bg-muted/20 p-2 ring-muted transition-colors will-change-auto hover:bg-muted/40 disabled:cursor-not-allowed",
