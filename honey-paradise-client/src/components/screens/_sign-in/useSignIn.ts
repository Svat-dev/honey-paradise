...
import { createSignInSchema, type TSignInFields } from "@schemas/sign-in.schema"
...
export const useSignIn = () => {
	...
	const { connectSocket, isTgSignInLoading } = useSessionWebsockets(msg =>
		onError(msg, true)
	)
	...
	const onSubmit = async (data: TSignInFields) => {
		...
		try {
			...
			if (tfa) {
				...
				setTimeout(
					() => replace(EnumAppRoute.SIGN_IN_CONFIRMATION),
					successDelay
				)
			} else {
				auth()
			...
			if (errCause === errorCauses.ACCOUNT_NOT_VERIFIED) {
				onError(errMsg)
				prefetch(
					`${EnumAppRoute.EMAIL_CONFIRMATION}&utm_source=${EnumAppRoute.SIGN_IN}`
				)
				return setTimeout(
					() =>
						replace(
	...
	useEffect(() => {
		if (
			...
			Cookies.get(EnumStorageKeys.SOCKET_SESSION_TOKEN)
		) {
			connectSocket()
			setDataStatus("good")
			setIsSuccess(true)
