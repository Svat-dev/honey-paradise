...
export const useNotificationsQueryParams = () => {
	...
	const isFilterUpdated = notificationsFilterStore(
		state => state.isFilterUpdated
	)
	const queryParams = notificationsFilterStore(state => state.queryParams)
	const updateQueryParam = notificationsFilterStore(
		state => state.updateQueryParam
	)
	const _reset = notificationsFilterStore(state => state.reset)

	const updateQueryParams = (
		key: keyof INotificationsQueryParams,
		value: string
	) => {
