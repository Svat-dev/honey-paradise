...
export const useNotificationsDM = (
	nid: string,
	isRead: boolean,
	isOpen: boolean
) => {
	const t = useTranslations(
		"global.notifications.content.notification.dropdown"
	)
	const { locale } = useLanguage(false)

	const { isSelectMode, selectedIds, setSelectMode, removeSelectedId } =
		useNotificationsContext()

	const { deleteNotification, markAsArchived, markAsRead } =
