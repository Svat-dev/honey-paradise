	...
	return (
		<div className={cn(styles["wrapper"], containerClassName)}>
			...
			{input_type === "radio-group" && (
				<RadioGroupInput name={name} data={data} isLoading={isLoading} />
			)}

			{input_type === "date" && (
				<DateInput name="birthdate" config={dateConfig} isLoading={isLoading} />
			)}

			{input_type === "otp" && (
				<OTPInput
					limit={otpSlotsLimit as number}
			...
			<AnimatePresence>
				{error && (
					<ErrorText
						...
						className={cn(styles["error-txt"], errorClassName)}
					/>
				)}
			</AnimatePresence>
		</div>
