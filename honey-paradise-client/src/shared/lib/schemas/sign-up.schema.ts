...
const passwordSchema = (t: any) =>
	z
		...
		.min(VALUES.MIN_PASSWORD_LENGTH, {
			message: t("main_part.form.password.errors.min", {
				min: VALUES.MIN_PASSWORD_LENGTH
			})
		})
		.max(VALUES.MAX_PASSWORD_LENGTH, {
			message: t("main_part.form.password.errors.max", {
				max: VALUES.MAX_PASSWORD_LENGTH
			})
		})
		.refine(validatePassword, {
			message: t("main_part.form.confirmPassword.errors.invalid")
		})
...
const usernameSchema = (t: any) =>
	z
		...
		.max(VALUES.MAX_ID_LENGTH, {
			message: t("optional_part.form.username.errors.max", {
				max: VALUES.MAX_ID_LENGTH
			})
		})
		.optional()
		.refine(data => (data ? data.length >= VALUES.MIN_ID_LENGTH : true), {
			message: t("optional_part.form.username.errors.min", {
				min: VALUES.MIN_ID_LENGTH
			})
		})
		.refine(validateUsername, {
			message: t("optional_part.form.username.errors.invalid", {
				min: VALUES.MIN_ID_LENGTH
			})
		})
...
export const createSignUpSchema = (t: any) =>
	z
		.object({
			email: z
				.string({ message: "" })
				.email({ message: t("main_part.form.email.errors.invalid") }),
