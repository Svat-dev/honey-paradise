...
export const useSessionWebsockets = (onError: (msg: string) => void) => {
	const { accept_callback, reject_callback, code_lifetime_expired_cb, isTgSignInLoading } = useWSTelegramAuth();
	...
	useEffect(() => {
		if (socket) {
			...
			socket.on(EnumWSRoutes.TG_CODE_LIFETIME_EXPIRED, msg => code_lifetime_expired_cb(msg, onError));
