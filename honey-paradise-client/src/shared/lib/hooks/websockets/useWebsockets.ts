...
export const useNotificationWebsockets = (
	...
	useEffect(() => {
		if (isAuthenticated) {
			socket?.on(EnumWSRoutes.NEW_NOTIFICATION, msg =>
				newNotificationCb(msg, socket, user?.notificationSettings)
			)
		...
		user?.notificationSettings.withSound,
		socket
	])
